...
class TrelloCli::Cli::Attach < Thor
  ...
  def get(ref, filename)
    ...
    output_name = attachment["fileName"] || filename
    output_path = options[:output] || File.join(Dir.pwd, output_name)

    say "Downloading #{output_name}..."

    data = TrelloCli::Api::Attachment.download(client, card_id, attachment)
